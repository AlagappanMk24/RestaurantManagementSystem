...
public class CustomerRepository(RestaurantDbContext dbContext) : GenericRepository<Customer>(dbContext), ICustomerRepository
{
    ...
    public async Task<List<Domain.Entities.RestaurantEntity>> GetFavoriteRestaurantsAsync(int customerId)
    {
        var customer = await dbContext.Customers
            .Include(c => c.FavoriteRestaurants)
            .FirstOrDefaultAsync(c => c.Id == customerId)
