...
{
    public class RestaurantsRepository(RestaurantDbContext dbContext) : GenericRepository<Domain.Entities.RestaurantEntity>(dbContext), IRestaurantRepository
    {
        public async Task<(IEnumerable<Domain.Entities.RestaurantEntity>, int)> GetAllMatchingAsync(string? searchPhrase,
            int pageSize,
            int pageNumber,
            string? sortBy,
            SortDirection sortDirection)
        {
            ...
            if (sortBy != null)
            {
                var columnsSelector = new Dictionary<string, Expression<Func<Domain.Entities.RestaurantEntity, object>>>
                {
                    { nameof(Domain.Entities.RestaurantEntity.Name), r => r.Name },
                    { nameof(Domain.Entities.RestaurantEntity.Description), r => r.Description },
                };
        ...
        public async Task<List<Domain.Entities.RestaurantEntity>> GetTopRatedAsync(int count)
        {
        ...
        public async Task<Domain.Entities.RestaurantEntity?> GetStatisticsAsync(int restaurantId)
        {
            return await dbContext.Restaurants
                .Include(r => r.Ratings)
                .Include(r => r.Dishes)
